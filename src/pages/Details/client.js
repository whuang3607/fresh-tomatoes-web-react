...
export const getMovieById = async (id) => {
  try {
    ...
    if (response.data == null) {
      const [movieResponse, actorsResponse, ratingsResponse] =
        await Promise.all([
      ...
      movie = {
        ...
        primary_image: movieResponse.data.results.primaryImage
          ? movieResponse.data.results.primaryImage
          : {
              url: 'https://www.dotyeti.com/wp-content/uploads/2023/01/barbie.webp',
            },
        actors: actorsResponse.data.results ? actorsResponse.data.results : [],
        rating: ratingsResponse.data.results
          ? ratingsResponse.data.results.averageRating
          : '0',
      };
    } else {
      movie = response.data;
